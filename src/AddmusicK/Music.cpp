...
void Music::parseHexCommand()
{
	...
	if (validateHex)
	{
		...
		else
		{
			...
			if (i == 0x00 || i == 0x06 || i == 0x0D)
				hasYoshiDrums = true; // NOTE: VCMD 0x0E also deals with Yoshi Drums, but it always disables them, hence there is no reason to have this trigger the Yoshi Drum check.
